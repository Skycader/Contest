...
const maxGrade = (N, staff, K) => {
	...
	for (let item of staff) {
		bestTeam.push(item)
		bestTeam.sort((a,b)=>(b-a))
		bestTeam = bestTeam.slice(0,K)

		let sum = bestTeam.reduce((total,item)=>total+=item,0)
		if (sum == 25*K) { console.log('break'); break }
	}
	console.log(bestTeam)
	return bestTeam.reduce((total,item)=>total+=item,0)
}
